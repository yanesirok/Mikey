...
#     0.549 -> 0.553) и лишь отчасти в cage (см. bake_pair_cloth). Главное — 12k
#     трисов low-поли не представляют 271k трисов десятислойной ткани: тот же
#     запек на low в 60k даёт синий 0.651 против 0.549 при прочих равных.
#
...
# Замерено на этой геометрии (1024², по текселям под островами):
#   cage 20.0 мм (мост): синий 0.544, >0.9 26.7%, <0.5 46.2%, промахов  2.9%
#   cage  1.0 мм:        синий 0.538, >0.9 29.3%, <0.5 46.8%, промахов  7.2%
#   cage  0.2 мм:        синий 0.726, >0.9 53.3%, <0.5 25.1%, промахов 24.8%
#   cage  0.0 мм:        синий 0.787, >0.9 63.4%, <0.5 19.1%, промахов 33.9%
# Взято 0.2 мм — лучшее измеренное отношение годных текселей к вредным (2.1:1
# против 0.58:1 у значений моста). Платим промахами, и это осознанный размен:
# промах оставляет тексель плоским, то есть ткань без рельефа, а неверный луч
# даёт нормаль внутрь, то есть вывернутое освещение. Уменьшать cage дальше
# смысла нет: разрыв low/high этим не лечится, лечится бюджетом трисов.
CLOTH_CAGE = 0.0002
CLOTH_RAY = 0.0008
