...
### Ловушка: двойной дефис в комментариях UXML

Разметка UXML — это XML, а в XML **двойной дефис внутри комментария недопустим**.
Имена классов этого проекта сплошь содержат его (`detail-panel--open`,
`chapter-node--locked`, `map-node--enter`), поэтому комментарий вида
«см. `.detail-panel--scroll`» ломает файл.

Отказ при этом бесшумный и обманчивый: тесты не падают с ошибкой разбора, а
**молча обнуляются** — файл перестаёт грузиться целиком, и проверки, которым
нечего проверять, проходят. Диагностируется разбором дерева документа.

В комментариях UXML имена классов писать без двойного дефиса — например через
одинарный или словами.
