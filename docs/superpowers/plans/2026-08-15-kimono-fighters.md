...
`WaterReflection` держит константу scale 0.296 и честно отражает бойцов. Смена модели её задевает. Отражение видно на кадре из Step 3 — отдельных действий не требуется, но в отчёте назови, видно ли отражение бойцов и не разъехалось ли оно по вертикали относительно них.
