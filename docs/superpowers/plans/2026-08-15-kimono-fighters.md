...
**Files:**
- Create: `Assets/Editor/FightSceneSwap.cs`
...
- [ ] **Step 1: Заменить модель бойцов скриптом**

Разведка сцены, проведённая до постановки задачи, — опирайся на неё, а не перепроверяй заново:

- `.prefab`-файлов в проекте нет вовсе. Бойцы — это **инстансы FBX-модели**: GUID `Ch15_nonPBR.fbx` встречается в сцене **184 раза**, GUID `KimonoFighter.fbx` — ноль раз. Поиск бойцов по `m_Name` их не находит, потому что имена инстансов живут в списках переопределений префаба; ищи по компоненту `Fighter`, а не по имени.
- `FightRound` в сцене уже ссылается на обоих: поля `player` и `enemy` заполнены ненулевыми fileID. Эти ссылки, как и взаимные `Fighter.opponent`, обязаны пережить свап.
- Компоненты бойца: `Animator`, `Fighter`, `FootIK` (требует `Animator`), плюс `PlayerFighterInput` (требует `Fighter`, ссылается на `FightTouchControls`) у игрока и `EnemyFighterAI` у врага. У обоих в сцене стоит `m_ApplyRootMotion: 0` — это переопределение обязано остаться нулём, на нём держится вся схема движения.
- Все нужные точки входа публичные и явно рассчитаны на сборку кодом: `FightRound.Player`, `FightRound.Enemy`, `Fighter.Opponent`.

Свап делается Editor-скриптом, а не мышью, по той же причине, что и в Task 5 и Task 6. Создать `Assets/Editor/FightSceneSwap.cs` с `public static void Run()` под `[MenuItem("Mikey/Swap Fighters To Kimono")]`.

Что он должен сделать для каждого из двух бойцов: заменить визуальную модель на `KimonoFighter.fbx`, сохранив на объекте бойца все перечисленные компоненты и их настройки, сохранив взаимные ссылки `Opponent` и ссылки из `FightRound`, сохранив позицию и поворот, и оставив `applyRootMotion` выключенным. Аватар у `Animator` переставить на аватар новой модели, иначе ретаргет молча перестанет работать. Материалы назначить так: на меш тела — общий `M_Fighter_Skin`, на меш кимоно — `M_Player_Kimono` игроку и `M_Enemy_Kimono` врагу.

Конкретный механизм выбери сам — Unity даёт несколько способов подменить источник инстанса, и какой из них корректно отработает на этой сцене, надёжнее выяснить на месте, чем предугадать. Требование одно: после прогона в сцене не должно остаться ни одной ссылки на GUID `Ch15_nonPBR`, а ссылки на `KimonoFighter` должны появиться. Это проверяемо grep'ом по файлу сцены, и это же требование стоит в тесте ниже.

Скрипт обязан сохранить сцену (`EditorSceneManager.SaveScene`), иначе правка не доживёт до выхода из редактора.

- [ ] **Step 2: Выполнить свап headless**

Через навык `unity-cli`, как в Task 5 и Task 6. Ожидание: exit 0 и строка о завершении в логе.

Затем проверить результат прямо по файлу сцены:

```bash
grep -c eb03c6ab06532004a8c0b75f7133ef1e Assets/Scenes/FightSandbox.unity   # Ch15, ожидание: 0
grep -c 1ab6969c14d7b5549b0b16c9e69fa959 Assets/Scenes/FightSandbox.unity   # KimonoFighter, ожидание: > 0
```

- [ ] **Step 3: Снять кадр и посмотреть на бойца**

В проекте уже есть `FightCapture.Shoot()` — `public static`, рендерит камеру сцены в PNG через RenderTexture, сам открывает `FightSandbox`. Аргументы: `-captureOut <файл.png>` и `-captureSize 1920x1080`.

Два условия, они в докстринге самого инструмента: запускать **без** `-nographics` (иначе нет GPU-контекста) и при **закрытом** редакторе (он держит project lock).

Снять кадр в `issues/kimono_fighters.png`.

- [ ] **Step 4: Снять кадр с ударом ногой**

Пробойность ткани проверяется на `Kick` — это самая нагруженная поза для штанины и подмышки. В Edit-режиме позу можно наложить без запуска игры: `clip.SampleAnimation(fighterGameObject, time)`, где clip — `YokoGeri_High`, а time — момент около пика удара. Добавь в скрипт свапа второй `public static` метод, который накладывает эту позу на обоих бойцов, после чего снова снимается кадр — в `issues/kimono_fighters_kick.png`.

Оба кадра посмотрю я. От тебя нужно, чтобы они были сняты и лежали по этим путям; выносить по ним вердикт «выглядит хорошо» не надо.

- [ ] **Step 5: Перепроверить отражение в воде**

`WaterReflection` отражает бойцов, поэтому смена модели её задевает — перепроверить после свопа. Оговорка: в первой редакции здесь стояла «константа scale 0.296». Такой константы в `WaterReflection.cs` нет; число было перенесено из более раннего плана проекта без сверки с кодом. В самом скрипте есть `resolutionScale = 0.5` и `mobileResolutionScale = 0.35`, но это масштабы разрешения render-texture, к выравниванию отражения они отношения не имеют. Отражение видно на кадре из Step 3 — отдельных действий не требуется, но в отчёте назови, видно ли отражение бойцов и не разъехалось ли оно по вертикали относительно них.

- [ ] **Step 6: Закрепить тестом, что root motion остаётся выключенным**
...
```bash
git add Assets/Editor/FightSceneSwap.cs Assets/Editor/FightSceneSwap.cs.meta Assets/Scenes/FightSandbox.unity Assets/Fight/Tests/FighterModelTests.cs
