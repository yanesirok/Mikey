...
- XML-комментарии в UXML **не могут содержать `--`**. Имена классов проекта содержат. Нарушение ломает разбор всего UXML: ассет продолжает грузиться, но отдаёт ПУСТОЕ дерево. Подпись отказа — лавина красных в чужих файлах, которые разбирают дерево (замерено: 76 падений в шести файлах), при зелёном своём, если свой читает разметку строкой. Число тестов при этом НЕ меняется, поэтому «набор опустел» детектором не является.
...
Ожидание: весь набор зелёный. Если в ЧУЖИХ файлах, разбирающих дерево
(`JapanMapScreenUxmlTests`, `OkinawaMapScreenUxmlTests`, `MapCloudAssetsTests`,
`MapMarkerAssetsTests`, `MapHudRedesignTests`, `SharedTopBarRedesignTests`),
внезапно посыпались десятки падений — значит, в комментарий попало `--`,
разбор UXML сломан и дерево пустое. Число тестов при этом НЕ изменится.
Проверить оба вставленных комментария.
