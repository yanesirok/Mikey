...
Свести пользователю: скриншот, строку `[diag deck]`, итоговую позицию камеры из лога сборки. Известные унаследованные ограничения (из HANDOFF): камера — латеральный ход только x ±6 относительно арены; отражение — самая дорогая часть кадра (scale 0.295858 в реальной сцене), первый кандидат на срез на слабом девайсе; бойцы на слое 8 и попадают в планарное отражение (унаследовано от старой сцены, оставлено осознанно — честное отражение в воде; цена — скиннед-меши рендерятся второй раз, при тормозах на девайсе первый кандидат на срез вместе с отражением).
