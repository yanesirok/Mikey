namespace Mikey.UI.Map
{
    ...
    /// Okinawa chapter map. Deliberately simple: zoom always pivots around the
    /// canvas's own center (its default USS transform-origin, which matches the
    /// viewport center since the canvas fills it) — no zoom-around-cursor/pinch-
    /// point math — and panning is clamped by a fixed fraction of the viewport
    /// size rather than pixel-exact content-edge alignment. A reliable, smooth
    /// system over a pixel-perfect one, per the mobile-map rebuild's brief.
    /// </summary>
