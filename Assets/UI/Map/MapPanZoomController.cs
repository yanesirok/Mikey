...
{
    /// <summary>
    /// Drives drag-to-pan and zoom on the Map's pannable ".map-canvas" layer
    /// (the map artwork + the LVL0/LVL1 hotspot markers, which move and scale
    /// together since they're all children of the same transformed element).
    /// Touch-first: a single pointer (mouse or one finger) pans via UI Toolkit's
    /// pointer events, which work uniformly for mouse and touch. Zoom has two
    /// independent sources — a mouse wheel (desktop/Editor) and two-finger pinch
    /// (device), read directly off <see cref="Touchscreen"/> since this project's
    /// active input handler is the Input System package, not the legacy
    /// UnityEngine.Input class. Deliberately simple (see
    /// <see cref="MapPanZoomMath"/>): zoom always pivots around the canvas's own
    /// center, and a short drag threshold distinguishes a tap on a hotspot button
    /// from an intentional pan, so pinch/drag never swallows a hotspot's click.
    /// Pan/zoom resets to the default on every fresh entry to the "map" screen.
    /// </summary>
    ...
    public sealed class MapPanZoomController : MonoBehaviour
    {
        ...
        private IEnumerator BindWhenReady()
        {
            ...
            _viewport = root.Q<VisualElement>("map-stage");
            _canvas = root.Q<VisualElement>("map-canvas");

            if (_viewport == null || _canvas == null)
            {
                Debug.LogError("[MapPanZoomController] Map canvas/viewport elements missing; pan/zoom not bound.", this);
                _bindRoutine = null;
                yield break;
            }
