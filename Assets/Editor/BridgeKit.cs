...
public static class BridgeKit
{
    ...
    // Экспорт (axis_forward=-Z, axis_up=Y) — это конверсия из системы Blender (Z — верх), а не
    // тождество: она переставляет Y<->Z, и переставляет их у ВСЕГО файла, а не у отдельных
    // деталей. Подтверждено баундами после импорта: Rail1m, авторски (1.0, 0.06, 0.08),
    // приходит как (1.00, 0.08, 0.06); Sill1m (1.0, 0.08, 0.14) — как (1.00, 0.14, 0.08);
    // PileBeam (1.45, 0.10, 0.20) — как (1.45, 0.20, 0.10). Правим поворотом на 90° вокруг X
    // (собственный поворот, det=+1 — винды треугольников и нормали остаются согласованными без
    // доп. правок): (x,y,z) -> (x,z,-y). Знак проверен диагностикой: у Pile сужение (r_top)
    // сидит на +Z, после поворота остаётся на +Y — верх остаётся верхом.
    //
    // Поворот безусловный. Сначала он стоял на трёх деталях, у которых перестановка ломала
    // длинную ось (Post/PostEnd/Pile) — и это чинило симптом, а не причину: у линейных деталей
